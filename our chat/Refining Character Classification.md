...
@[conversation:"Classification Fixes & Text Integrity"] موافق  في حالة واحدة انك تعمل فحص كامل للتطبيق تشوف اللي  انت هتطبق  هل هو بيتعارض مع حاجة او بيبوظ حاجة تانية او اصلا ممكن يكون متطبق بس بأسم تاني و تقدملي تقرير تدمجه مع الخطة 

### Planner Response

هبدأ فحص شامل للتطبيق دلوقتي. هشوف:
1. هل الـ Character Registry موجود بالفعل بأي شكل؟
2. هل `splitMergedActionCharacter` بيتعارض مع [mergeBrokenCharacterName](file:///e:/mo7rer/src/extensions/line-repair.ts:152:0-206:2)؟
3. هل فيه أي حاجة هتتكسر؟

*Viewed [context-memory-manager.ts](file:///e:/mo7rer/src/extensions/context-memory-manager.ts) *

### Planner Response

**اكتشاف مهم!** الـ [ContextMemoryManager](file:///e:/mo7rer/src/extensions/context-memory-manager.ts:130:0-455:1) عنده فعلاً `commonCharacters` و `characterDialogueMap`. خليني أتتبع كيف بيتم استخدامهم.
...
اكتشاف مهم جداً. خليني أتحقق من حاجة حرجة:
...
الأسطر **مش مدمجة من الـ backend** — الـ **[mergeBrokenCharacterName](file:///e:/mo7rer/src/extensions/line-repair.ts:152:0-206:2) هو اللي بيدمجها غلط!**

خليني أتتبع بالظبط:

**سطر N:** `مازال لؤي يتحدث فى الهاتف` → classified ACTION ✅

**سطر N+1:** `لؤي:` → يدخل [mergeBrokenCharacterName](file:///e:/mo7rer/src/extensions/line-repair.ts:152:0-206:2):
- `prevNamePart = "مازال لؤي يتحدث فى الهاتف"` → **25 حرف = ≤25** → يعدّي!
- `currNamePart = "لؤي"` → كلمة واحدة ≤3 → يعدّي!
- المجموع 28 ≤ 32 → يعدّي!
- `CHARACTER_RE` يطابق (≤30 بعد أول حرف) → يعدّي!
- **النتيجة:** يدمجهم ويرجع `"مازال لؤي يتحدث فى الهاتفلؤي:"` → CHARACTER! ❌

**نفس الحاجة مع** `ثم تنظر الى الدور العلوي` + `اميرة:` → 24 حرف ≤25 → يدمج غلط!

**الثغرة:** [mergeBrokenCharacterName](file:///e:/mo7rer/src/extensions/line-repair.ts:152:0-206:2) عنده limit على **طول** الـ prevNamePart (25 حرف) لكن **مفيش limit على عدد الكلمات** ومفيش **فحص أفعال**. اسم شخصية مقسوم مستحيل يكون 5 كلمات!
...
الملف اتلخبط بسبب الـ edit. هكتبه من الأول نضيف:

*Edited relevant file*

### Planner Response

الخطة جاهزة بالتقرير الكامل. ملخص النتائج:

### أهم اكتشافات الفحص:

1. **[mergeBrokenCharacterName](file:///e:/mo7rer/src/extensions/line-repair.ts:152:0-206:2) هو السبب الجذري** — مش الـ backend. الـ classifier نفسه بيدمج `"مازال لؤي يتحدث فى الهاتف"` (25 حرف) + `"لؤي:"` → CHARACTER لأن مفيش word count limit على الجزء الأول

2. **Character Registry موجود** في [ContextMemoryManager](file:///e:/mo7rer/src/extensions/context-memory-manager.ts:130:0-455:1) (`commonCharacters` + `characterFrequency`) — بس [isCharacterLine](file:///e:/mo7rer/src/extensions/character.ts:213:0-239:2) بيشتغل **قبل** [HybridClassifier](file:///e:/mo7rer/src/extensions/hybrid-classifier.ts:49:0-129:1) فمش بيستفيد منه

3. **`splitMergedActionCharacter` مش محتاج** — المشكلة مش في الاستخراج

4. **لا تعارض** مع أي كود موجود

### الخطة:
1. **إصلاح [mergeBrokenCharacterName](file:///e:/mo7rer/src/extensions/line-repair.ts:152:0-206:2)** — إضافة word count limit + action verb check (سطرين)
2. **تفعيل الـ Registry** — two-pass scan قبل الـ classification loop
3. اختبار + idol file

موافق على الخطة النهائية دي؟
...
Fix 1 done ✅. الآن Fix 2: Two-pass registry scan.
...
الآن Fix 2: تحديث [isCharacterLine](file:///e:/mo7rer/src/extensions/character.ts:213:0-239:2) لقبول registry + إنشاء `buildCharacterRegistry`.
...
خطأ في التعديل — لازم أصلح structure الـ file.
...
تم إصلاح structure الملف. الآن لازم أضيف import في [paste-classifier.ts](file:///e:/mo7rer/src/extensions/paste-classifier.ts:0:0-0:0) وأستخدم الـ registry.
...
**الملفات المعدّلة:**
1. [line-repair.ts](file:///e:/mo7rer/src/extensions/line-repair.ts:0:0-0:0) — إضافة word count limit + action verb check في [mergeBrokenCharacterName](file:///e:/mo7rer/src/extensions/line-repair.ts:158:0-224:2)
2. [character.ts](file:///e:/mo7rer/src/extensions/character.ts:0:0-0:0) — إضافة [buildCharacterRegistry](file:///e:/mo7rer/src/extensions/character.ts:213:0-244:2) + تحديث [isCharacterLine](file:///e:/mo7rer/src/extensions/character.ts:246:0-279:2) 
3. [paste-classifier.ts](file:///e:/mo7rer/src/extensions/paste-classifier.ts:0:0-0:0) — استدعاء [buildCharacterRegistry](file:///e:/mo7rer/src/extensions/character.ts:213:0-244:2) وتمريره لـ [isCharacterLine](file:///e:/mo7rer/src/extensions/character.ts:246:0-279:2)

الآن commit و push:

*User accepted the command `git status`*

*User accepted the command `git add src/extensions/character.ts src/extensions/line-repair.ts src/extensions/paste-classifier.ts`*

*User accepted the command `git commit -m "fix: resolve classification issues in 1990.doc - mergeBrokenCharacterName + Character Registry

- Fix mergeBrokenCharacterName: add word count limit (>3 words reject) and action verb check
  to prevent merging narrative text with character names (e.g., 'مازال لؤي يتحدث' + 'لؤي:')

- Add buildCharacterRegistry function: two-pass scan to build character names from inline patterns

- Update isCharacterLine: accept knownCharacters registry for positive identification

- Update classifyLines: build registry before loop and pass to isCharacterLine

- Prevent 'ايوة:' from being classified as CHARACTER when not in registry"`*

*Checked command status*

*User accepted the command `git push -u origin مرحلة-2`*
...
**Commit:** `b154089` — fix: resolve classification issues in 1990.doc  
**Branch:** `مرحلة-2` pushed to origin

### التغييرات:
| ملف | التعديل |
|-----|---------|
| [line-repair.ts](file:///e:/mo7rer/src/extensions/line-repair.ts:0:0-0:0) | [mergeBrokenCharacterName](file:///e:/mo7rer/src/extensions/line-repair.ts:158:0-212:2): word count limit + action verb check |
| [character.ts](file:///e:/mo7rer/src/extensions/character.ts:0:0-0:0) | [buildCharacterRegistry](file:///e:/mo7rer/src/extensions/character.ts:213:0-244:2) + [isCharacterLine](file:///e:/mo7rer/src/extensions/character.ts:213:0-246:2) مع registry parameter |
| [paste-classifier.ts](file:///e:/mo7rer/src/extensions/paste-classifier.ts:0:0-0:0) | استدعاء registry قبل الـ loop + تمريره لـ [isCharacterLine](file:///e:/mo7rer/src/extensions/character.ts:213:0-246:2) |

### جاهز للاختبار:
افتح `1990.doc` وحاول paste نفس الملف وقارن النتائج. المفروض:
- `مازال لؤي يتحدث فى الهاتف` → ACTION ✅
- `لؤي:` → CHARACTER ✅  
- `ايوة:` → مش CHARACTER (مش في الـ registry) ✅

لو النتيجة تمام نعمل idol file.
...
<div data-type="action" class="screenplay-action">تقدر تاخد مدام اميرة والباقيين يا استاذ مصطفى ..</div>
<div data-type="action" class="screenplay-action">تحبس المتهة نهال سماحة .. خمسة عشر يوما على ذمة التحقيق</div>
<div data-type="character" class="screenplay-character">عايز:</div>
<div data-type="action" class="screenplay-action">فيلا رأفت – الهول</div>
<div data-type="character" class="screenplay-character">اميرة:</div>
<div data-type="dialogue" class="screenplay-dialogue">وما:</div>
<div data-type="dialogue" class="screenplay-dialogue">بلغتش ليه ؟</div>
<div data-type="character" class="screenplay-character">امجد:</div>
<div data-type="character" class="screenplay-character">ابلغ:</div>
<div data-type="dialogue" class="screenplay-dialogue">ليه .. الراجل ابوه مقتول.. وعايز يعرف مين اللي قتله ما يمكن اللي قتله هو اللي قتل رنا وهو اللي بعتلكوا السي دي</div>
<div data-type="action" class="screenplay-action">يادي النيلة .. والزفتة مش موجودة</div>
@[conversation:"Classification Fixes & Text Integrity"] 
الجمل  دي كلها اخطاء لسه @[conversation:"Classification Fixes & Text Integrity"] 

### Planner Response

هراجع دلوقتي من جذر المشكلة ليه `ايوة:` و `عايز:` و `ابلغ:` لسه بيتصنفوا `character`، وهنفصل ده لـ:
- **تشخيص**: فين المنطق اللي لسه بيقبل الكلمات دي كـ character
- **تعديل محدد**: بدون ما نغيّر المتن، ونمنع hardcoding على أمثلة بعينها
وبعدها هنشغّل build/check سريع عشان نضمن مفيش TypeScript errors. هبدأ بقراءة أجزاء [character.ts](file:///e:/mo7rer/src/extensions/character.ts:0:0-0:0) اللي بتحدد [isCandidateCharacterName](file:///e:/mo7rer/src/extensions/character.ts:84:0-110:2) و `SHORT_DIALOGUE_WORDS`.
...
تمام. اللي ظاهر من الـ HTML اللي بعته إن لسه فيه **سطر واحد بكلمة + `:`** بيتصنف `character` (زي `ايوة:` / `عايز:` / `ابلغ:`)، وده معناه إن منطق الـ registry اللي عملناه **لسه بيعتبر أي كلمة مجهولة وتنتهي بـ `:` اسم شخصية** (أو الأسوأ إن الـ registry نفسه اتملّى بكلمات زي دي). هعمل دلوقتي فحص سريع لجذر المشكلة في [buildCharacterRegistry](file:///e:/mo7rer/src/extensions/character.ts:213:0-244:2) و [isCharacterLine](file:///e:/mo7rer/src/extensions/character.ts:246:0-279:2)، وبعدها هطبّق تعديل عام يمنع أي “كلمة واحدة + :” من إنها تبقى `character` إلا لو اتثبتت كاسم شخصية (inline dialogue أو دليل حوار بعده).
...
*Viewed [character-first-appearance.test.ts](file:///e:/mo7rer/tests/unit/extensions/character-first-appearance.test.ts) *
...
هنقفل ده جذرياً: الأخطاء اللي انت عرضتها معناها إن “كلمة واحدة + `:`” لسه بتعدّي كـ `character` (وأحياناً كمان بتدخل غلط جوّه الـ registry). هقرأ دلوقتي `hybrid-classifier.ts` و `classification-decision.ts` و جزء `paste-classifier` اللي المفروض بيعمل guard لـ single-token first appearance (فيه test موجود أصلاً)، وبعدين هعدّل المنطق بحيث **أي كلمة واحدة منتهية بـ `:` ما تتقبلش كـ character إلا لو اتأكدت** (registry من inline + تكرار في الذاكرة).
...
*Viewed [paste-classifier.resilience.test.ts](file:///e:/mo7rer/tests/unit/extensions/paste-classifier.resilience.test.ts) *
...
هشتغل دلوقتي كالتالي: هحدّد **السبب الجذري بالظبط** لليه `ايوة:` و`عايز:` و`ابلغ:` لسه بيتصنفوا `character` رغم تغييراتنا، وبعدها هحدّث **ملف الخطة** بنفسه بخطوات إصلاح عامة (بدون ما نلمس المتن)، لأن المطلوب حالياً هو تحديث الخطة فقط قبل أي تعديلات كود جديدة.

*Edited Jupyter notebook*

### Planner Response

هصلّح الخطأ اللي حصل: أنا استخدمت أداة غلط (`edit_notebook`) على ملف `.md`. دلوقتي هحدّث **ملف الخطة نفسه** بـ `apply_patch` عشان يوثّق السبب الجذري الجديد اللي ظاهر من الـ HTML اللي بعته (تلوث الـ registry + قبول single-token `:`) + خطة إصلاح عامة بدون ما نغيّر المتن.
...
الـ HTML اللي بعته بيثبت إن لسه فيه **٣ جذور مشاكل** بتطلع نفس نوع الأخطاء:

## التشخيص (Root Cause) على الأمثلة اللي عندك

### 1) **[buildCharacterRegistry](file:///e:/mo7rer/src/extensions/character.ts:213:0-244:2) متلوّث**
- **السلوك المرصود:** `ايوة:` و`عايز:` و`ابلغ:` اتحطّوا `character`.
- **المتوقع:** دول **حوار/أفعال** مش أسماء شخصيات.
- **فين الفشل؟** في [character.ts](file:///e:/mo7rer/src/extensions/character.ts:0:0-0:0) عند [buildCharacterRegistry](file:///e:/mo7rer/src/extensions/character.ts:213:0-244:2):
  - حالياً بيضيف **أي سطر منتهي بـ `:`** للـ registry، مش بس inline dialogue.
  - فـ كلمة زي `ايوة:` بتدخل `knownCharacters` بالغلط.
- **ليه بيفشل؟** لأن [isCharacterLine](file:///e:/mo7rer/src/extensions/character.ts:246:0-279:2) عامل short-circuit:
  - لو الاسم موجود في `knownCharacters` → بيرجع `true` فوراً.

### 2) **قبول “كلمة واحدة + :” كـ Character بدون إثبات**
- **السلوك المرصود:** `وما:` و`ابلغ:` ظهروا كـ `dialogue/character` بشكل بيكسر تسلسل الحوار.
- **المتوقع:** جوّه كتلة الحوار (بعد `اميرة:` / `امجد:`) المفروض دول يبقوا **dialogue** مش `character`.
- **فين الفشل؟** [isCandidateCharacterName](file:///e:/mo7rer/src/extensions/character.ts:84:0-110:2) بيقبل كلمات عربية مفردة كتير طالما “شكلها اسم” (مفيش ترقيم جملة… إلخ).
- **ليه بيفشل؟** مفيش Guard عام يمنع single-token `:` من إنه يعمل short-circuit كـ character إلا لو اتأكد.

> فيه كمان test موجود في المشروع اسمه [character-first-appearance.test.ts](file:///e:/mo7rer/tests/unit/extensions/character-first-appearance.test.ts:0:0-0:0) بيؤكد إن لازم يبقى فيه guard للنوع ده.

### 3) **حوار بيتصنف Action لما يبدأ بفعل (خصوصاً بعد character)**
- **السلوك المرصود:**  
  `تقدر تاخد…` و`تحبس…` متصنفين `action`  
  و`يادي النيلة…` متصنف `action`
- **المتوقع:** في السياق ده غالباً **dialogue**.
- **فين الفشل؟** في [dialogue.ts](file:///e:/mo7rer/src/extensions/dialogue.ts:0:0-0:0) → [isDialogueLine](file:///e:/mo7rer/src/extensions/dialogue.ts:138:0-185:2) بيكسر الحوار بمجرد `collectActionEvidence.byVerb` (وده واسع، لأن أي كلمة تبدأ بـ ت/ي/ن/أ ممكن تتاخد “فعل”).
- **ليه بيفشل؟** لأن القاعدة الحالية مش بتراعي كفاية إن “بعد character” ده سياق حوار قوي.

---

## اللي اتعمل دلوقتي
- أنا **حدّثت ملف الخطة** ودمجت التشخيص الجديد + خطوات إصلاح عامة (بدون أي تعديل على متن الكاتب) هنا:
`C:\Users\Mohmed Aimen Raed\.windsurf\plans\phase2-classification-fixes-1d272f.md`

وفيه دلوقتي ٤ إصلاحات واضحة:
- **تنظيف الـ registry**: يبقى من inline patterns فقط
- **Guard عام** ضد single-token `:`
- **تقوية الحوار بعد character** لما يبدأ بفعل
- **تشغيل اختبارات** قبل اختبار 1990 + idol

---

## المطلوب منك قبل التنفيذ
اقرأ الخطة المحدّثة، ولو موافق على الإصلاحات (خصوصاً Fix 3 و Fix 4) رد بكلمة: **“موافق”** عشان أطلع من وضع التخطيط وأبدأ تنفيذ التعديلات على الكود وتشغيل الاختبارات.