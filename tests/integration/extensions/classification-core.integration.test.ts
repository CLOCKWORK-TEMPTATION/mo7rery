...
describe("classification-core integration", () => {
  ...
  it("يصنف 'بوسي : بسم الله الرحمن الرحيم' كـ character + dialogue وليس basmala", () => {
    logTestStep("basmala-inline-dialogue-not-basmala");
    const classified = classifyText("بوسي : بسم الله الرحمن الرحيم");
    const types = classified.map((line) => line.type);

    expect(types).not.toContain("basmala");
    expect(types).toContain("character");
    expect(types).toContain("dialogue");
  });

  it("يصنف بسملة مستقلة كـ basmala", () => {
    logTestStep("standalone-basmala-still-works");
    const classified = classifyText("بسم الله الرحمن الرحيم");
    expect(classified.length).toBe(1);
    assertClassificationType(classified[0], "basmala");
  });
});
