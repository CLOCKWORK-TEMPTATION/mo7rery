...
import { CHUNK_SIZE, CHUNK_OVERLAP, logger } from './config.js';
...
function splitLargeText(text: string, maxChars: number, overlap: number): string[] {
  const chunks: string[] = [];
  let start = 0;
...
export function getAllCodeFiles(rootDir: string): string[] {
  const files: string[] = [];
  const excludeDirs = ['node_modules', 'dist', 'build', '.git', 'coverage', 'test-results'];
