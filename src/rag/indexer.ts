import { qdrantClient, RAG_COLLECTION_NAME, EMBEDDING_DIMENSIONS, logger } from './config.js';
import { getAllCodeFiles, chunkFile } from './chunker.js';
...
export async function createCollection(): Promise<void> {
  try {
    ...
    const exists = collections.collections.some((c) => c.name === RAG_COLLECTION_NAME);

    if (exists) {
      logger.info(`Collection "${RAG_COLLECTION_NAME}" already exists, deleting...`);
...
export async function indexCodebase(rootDir: string): Promise<void> {
  try {
    ...
    logger.info(`Generated ${allChunks.length} chunks from ${files.length} files`);
