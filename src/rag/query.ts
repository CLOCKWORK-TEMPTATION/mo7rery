import { qdrantClient, RAG_COLLECTION_NAME, GEMINI_API_KEY, logger } from './config.js';
import { generateEmbedding } from './embeddings.js';
import { SearchResult, RagResponse } from './types.js';
...
أجب على السؤال بناءً على الكود أعلاه.`;
    ...
    try {
      ...
      return { answer, sources: searchResults.map((result) => ({
      ...
      return { 
        answer: 'فشل في الاتصال بـ Gemini. النتائج المتاحة:\n\n' + 
          searchResults.map((r, i) => `${i + 1}. ${r.chunk.metadata.filePath}\n${r.chunk.content.slice(0, 300)}...`).join('\n\n'),
