    ...
    if (hasScene1 || hasScene2) {
      ...
      if (m1) {
        ...
        operations.push({
          op: "SPLIT",
          at: idx,
          detail: `Extract SCENE-HEADER-1 from: "${norm}"`,
        });
      ...
      if (m2) {
        ...
        operations.push({
          op: "SPLIT",
          at: idx,
          detail: `Extract SCENE-HEADER-2 from: "${norm}"`,
        });
      ...
      if (possibleLocation && possibleLocation.length >= 3) {
        ...
        if (!startsWithVerb && !looksSpeaker) {
          operations.push({
            op: "SPLIT",
            at: idx,
            detail: `Extract SCENE-HEADER-3 from same line: "${norm}"`,
          });
          push("SCENE-HEADER-3", raw, possibleLocation, 0.8, [
