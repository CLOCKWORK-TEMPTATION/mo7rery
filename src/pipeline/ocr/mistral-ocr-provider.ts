...
export class MistralOcrProvider implements OcrProvider {
    ...
    constructor(opts?: { apiKey?: string; model?: string; endpoint?: string }) {
        ...
        this.model = opts?.model || process.env.MISTRAL_OCR_MODEL || "mistral-ocr-latest";
        this.endpoint = opts?.endpoint || process.env.MISTRAL_OCR_ENDPOINT || "https://api.mistral.ai/v1/ocr";
        if (!this.apiKey) throw new Error("MISTRAL_API_KEY is required for MistralOcrProvider");
    }
