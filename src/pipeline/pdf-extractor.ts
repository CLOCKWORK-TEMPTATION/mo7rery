...
import { getSuspiciousPages, countSuspiciousLines } from "./quality/suspicious-detector.js";
...
import type { DocumentExtraction, OcrProvider, PageExtraction, PipelineConfig } from "./types.js";
...
export async function runPdfTextLayerFirstPipeline(args: {
    ...
    const suspiciousBefore = countSuspiciousLines(pages, config.quality.suspiciousThreshold);

    let pagesSentToOcr: number[] = [];
    let patchedLines = 0;

    if (config.ocr.enabled && args.ocrProvider && config.ocr.provider !== "none") {
        pagesSentToOcr = getSuspiciousPages(pages, config.quality.suspiciousThreshold);
    ...
    const suspiciousAfter = countSuspiciousLines(pages, config.quality.suspiciousThreshold);
    ...
    const doc: DocumentExtraction = {
        ...
        stats: {
            ...
            pagesWithTextLayer: extracted.pages.filter((p) => p.items.length > 0).length,
