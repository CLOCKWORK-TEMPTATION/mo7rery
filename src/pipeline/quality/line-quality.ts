...
export function scoreLine(text: string): LineQuality {
    ...
    const hasBrokenArabicPattern = BROKEN_ARABIC_PATTERNS.some((rx) => rx.test(text));
    const suspiciousDialoguePattern = SUSPICIOUS_DIALOGUE_PATTERNS.some((rx) => rx.test(text));
