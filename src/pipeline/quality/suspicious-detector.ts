...
export function getSuspiciousPages(pages: PageExtraction[], threshold = 0.62): number[] {
    const out: number[] = [];

    for (const p of pages) {
        const suspicious = p.lines.filter((l) => l.quality.score < threshold).length;
...
export function countSuspiciousLines(pages: PageExtraction[], threshold = 0.62): number {
    return pages.reduce(
