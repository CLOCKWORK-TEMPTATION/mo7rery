...
export function validateArabicScreenplayLine(line: ExtractedLine): string[] {
    ...
    // حوار ناقص
    if (/^[^:]{1,40}\s*[:：]\s*$/.test(t)) flags.push("screenplay_incomplete_dialogue");
    ...
    if (/^[^:]{1,40}\s*[:：]\s+.*\b(يدخل|يخرج|ينهض|يجلس|تقف|تجلس|ينظر|يمشي)\b/.test(t)) {
...
export function applyScreenplayValidation(lines: ExtractedLine[]): ExtractedLine[] {
