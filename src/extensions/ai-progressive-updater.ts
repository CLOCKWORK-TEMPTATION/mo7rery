 ...
 * - {@link AILayerSource} — مصدر الطبقة (gemini-context / kimi-doubt / claude-review)
...
export class ProgressiveUpdateSession {
  ...
  private _applyToProseMirror(
    ...
    try {
      ...
      if ((targetNode as PmNode).type === newNodeType) return false;

      // إنشاء node جديد بنفس المحتوى
      const content = (targetNode as PmNode).content;
      const attrs = (targetNode as PmNode).attrs;

      // sceneHeaderTopLine حالة خاصة — ممكن يكون عنده children مختلفين
      if (command.correctedType === "sceneHeaderTopLine") {
        // لو بنحوّل لـ sceneHeaderTopLine محتاجين header1 + header2
        // ده حالة معقدة — نتجاهلها هنا ونسيبها لـ Claude
        return false;
      }
