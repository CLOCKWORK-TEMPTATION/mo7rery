...
/**
 * الأنواع المُشاركة في Viterbi (بدون sceneHeaderTopLine اللي بيتكون من header1+header2).
...
const buildTransitionMatrix = (): Float64Array => {
  ...
  set("basmala", "sceneHeaderTopLine", 2.0);
  set("basmala", "action", 1.0);
  set("basmala", "character", 0.5);

  // ─── sceneHeaderTopLine (بعده action أو character)
  set("sceneHeaderTopLine", "action", 2.0);
  set("sceneHeaderTopLine", "character", 1.5);
  set("sceneHeaderTopLine", "sceneHeader3", 1.0);
  set("sceneHeaderTopLine", "transition", 0.0);
  set("sceneHeaderTopLine", "sceneHeaderTopLine", -2.0);

  // ─── sceneHeader3 (تفصيل الموقع — بعده action أو character)
  set("sceneHeader3", "action", 2.0);
  set("sceneHeader3", "character", 1.5);

  // ─── action (الأكثر مرونة)
  set("action", "action", 1.0);
  set("action", "character", 1.5);
  set("action", "transition", 0.5);
  set("action", "sceneHeaderTopLine", 0.5);
  ...
  set("dialogue", "sceneHeaderTopLine", 0.0);
  ...
  set("transition", "sceneHeaderTopLine", 2.0);
...
const computeEmissionScores = (f: SequenceFeatures): Float64Array => {
  ...
  // ─── sceneHeaderTopLine
  {
    const idx = TYPE_INDEX.get("sceneHeaderTopLine")!;
    if (f.wordCount <= 10 && !f.endsWithColon) {
      scores[idx] += 0.5;
    } else {
      scores[idx] -= 3.0;
    }
  ...
  // ─── sceneHeader3
  {
    const idx = TYPE_INDEX.get("sceneHeader3")!;
...
const viterbiDecode = (
  ...
  for (let t = 1; t < n; t++) {
    ...
    for (let s = 0; s < k; s++) {
      ...
      for (let r = 0; r < k; r++) {
        const score =
          prob[prevOffset + r] + TRANSITION_MATRIX[r * k + s] + allEmissions[t][s];
        if (score > bestScore) {
          bestScore = score;
          bestPrev = r;
        }
