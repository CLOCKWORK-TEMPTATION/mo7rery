...
const resolveContextEndpoint = (): string => {
  const envValue = (
    import.meta.env.VITE_AI_CONTEXT_ENDPOINT as string | undefined
  )?.trim();
...
const isContextLayerEnabled = (): boolean => {
  const rawValue = (
    import.meta.env.VITE_AI_CONTEXT_ENABLED as string | undefined
  )
    ?.trim()
    .toLowerCase();
...
export const requestContextEnhancement = async (
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Context enhance failed (${response.status}): ${errorText}`);
    }
