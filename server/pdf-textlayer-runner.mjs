...
const runPdfTextLayerScript = async (
    inputPdfPath,
    textOutputPath
) =>
...
export const isPdfTextLayerScriptAvailable = () =>
    existsSync(CLI_SCRIPTS_PATH);
...
export async function runPdfTextLayerFlow(buffer, filename) {
    ...
    try {
        ...
        let stdoutText = "";
        if (err?.stdout) stdoutText = new TextDecoder("utf-8").decode(err.stdout).trim();

        let stderrText = "";
        if (err?.stderr) stderrText = new TextDecoder("utf-8").decode(err.stderr).trim();
        ...
        throw new Error(
            `فشل تحويل ملف PDF عبر textlayer API${warnings.length > 0 ? ` | ${warnings.join(" | ")}` : ""
            }`,
            {
                cause: error,
            }
